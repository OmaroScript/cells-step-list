 ...
 *
 * Inline lev el HTML tags are allowed for the message. Also custom content can be used as light DOM.
 *
 * __Example:__
 *
 * ```html
...
class CellsIconMessage extends Polymer.mixinBehaviors([ CellsBehaviors.i18nBehavior ], Polymer.Element) {
  ...
  static get properties() {
    return {
      ...
      /**
       * List of Text messages. Inline HTML elements are allowed (`<strong>`, `<b>`, `<em>`, `<i>`, etc.)
       */
      messages: {
        type: Array,
      },
      ...
      /**
       * flag that indicate if message is inline with the slot content
       */
      inlineContentWithText: {
        type: Boolean,
        value: false
      },

      /**
       * Flag to hide/show the component from the outside (enabling to do it with channel comunication)
       */
      hidden: {
        type: Boolean,
        value: false
  ...
  _observeSizes(sizes) {
    let width = this.get('value.width', sizes);
    let height = this.get('value.height', sizes);
